...
#include "../Config.hpp"
#include "../counters/CRCounters.hpp"
   ...
   namespace cr
   {
      ...
      void Worker::walEnsureEnoughSpace(u32 requested_size)
      {
         if (FLAGS_wal)
         {
            ...
            if (walContiguousFreeSpace() < (requested_size + CR_ENTRY_SIZE))
      ...
      //          if (FLAGS_vw && FLAGS_vw_todo && todo_list.size())
      //          { // Cleanup
      //             {
      ...
      {
         all_workers[worker_id]->getWALEntry(lsn, in_memory_offset, [&](WALEntry *entry)
                                             { callback(reinterpret_cast<WALDTEntry *>(entry)->payload); });
      }
      // -------------------------------------------------------------------------------------
