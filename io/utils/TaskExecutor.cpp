...
#include "../counters/ThreadCounters.hpp"
      ...
      while (_keep_running)
      {
         ...
         */
         ...
         DEBUG_TASK_COUNTERS_BLOCK(
             ...
             if (subDration > leanstore::ThreadCounters::myCounters().exec_cycl_max_task_us) { leanstore::ThreadCounters::myCounters().exec_cycl_max_subm_us = subDration; } counters.ioSubDuration += subDration;
         ...
         //*
         Task *task;
         while (tasksRun < maxTasksRun && popTask(task))
         { // pop after maxTaskRun check
         ...
         if (tasksRun == 0)
         {
            ...
            if (cyclesNothingRun > sleepIfNothingRunForCycles)
            {
               ...
               if (waitingTaskCount > 0 && cyclesNothingRun == sleepIfNothingRunForCycles + 5000)
               { // thread did not move in 5 seconds
                  // ensure(false);
         ...
         DEBUG_TASK_COUNTERS_BLOCK(
             ...
             if (taskDuration > leanstore::ThreadCounters::myCounters().exec_cycl_max_task_us) { leanstore::ThreadCounters::myCounters().exec_cycl_max_task_us = taskDuration; } if (totalDuration > leanstore::ThreadCounters::myCounters().exec_cycl_max_dur) { leanstore::ThreadCounters::myCounters().exec_cycl_max_dur = totalDuration; }
             /*
